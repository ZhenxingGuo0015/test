...
# Using TRESS for m6A peak calling

##  Input data preparation
TRESS starts with paired input and IP BAM files from each replicate. 
The input and IP BAM files are taken in the same order by 
arguments ``Input.file`` and ``IP.file``, 
i.e., "input1.bam", "input2.bam", ... for ``Input.file``; 
"ip1.bam", "ip2.bam", .... for ``IP.file``. 
For peak calling, these BAM files are paired input \& IP 
replicates for samples from one condition. 
For differential peak calling, 
they are paired input \& IP replicates 
...
Depending on the number of input \& IP replicates, TRESS
calls different functions to detect transcriptome-wide 
m6A regions. 
If there are multiple replicates, 
TRESS first obtains candidate regions with an 
ad hoc algorithm and then detects and 
ranks significant m6A regions among the candidates. 
If there is only one replicate, candidate regions from 
the first step are the final peak list.

##  Multiple replicates 
### Bin-level read counts and bump finding
Given all BAM files and the TXDB file, 
TRESS first divides each gene 
into 50bp long bins and calculate read counts in each bin. 
...
Bumps from all input\& IP replicates are combined together to 
...
### Detect and rank significant m6A regions from candidates
...
estimate library size factor (one component of ``Candidates``) 
for each sample. 
...
For data with only one replicate, TRESS calls function ``CallPeaks.oneRep()`` 
...
## Visualization of individual peaks
...
# Using TRESS for differential peak calling 
Unlike peak calling which implements all steps in one function
``TRESS_peak()``, TRESS separates the model fitting (implemented 
by function ``TRESS_DMRfit()``), which is the most 
computationally heavy part, from the hypothesis testing 
(implemented by function ``TRESS_DMRtest()``). 
Given an experimental design with multiple factors, 
the parameter estimation (model fitting) only 
...
_**A word about ``TRESS_DMRfit()``**_
It conducts a two-step approach to model transcriptome data. 
In the first step, it divides the whole genome into bins of 
fixed length (by subfunction ``DivideBins()``) 
and loosely calls candidate DMRs from samples across 
all conditions using an ad hoc bump-finding algorithm 
(by subfunction ``CallCandidates()``). 
In the second step, it models candidate DMRs 
based on Negative binomial distribution and 
estimates all parameters for each candidate DMR 
(by subfunction ``CallDMRs.paramEsti()``). 
Please find the manual pages for detailed usage of all three
subfunctions. 
...
## Input data preparation
For model fitting, 
TRESS requires paired input and IP (“input1.bam & ip1.bam”, 
“input2.bam & ip2.bam”, …, “inputN.bam & ipN.bam”) BAM files 
listed in order for samples from different conditions. 
Reference genome annotation file saved in 
``*.sqlite`` format is also required to obtain trancriptome 
bin- and region-level data.
As the methylation of each candidate is linked
to multiple factors in experimental design, then 
a ``variable`` data frame containing the condition 
information of all samples, and a particular ``model`` 
determining which factor will be included 
should also be provided construct a design matrix 
(``model.matrix(model, variable)``). 

For hypothesis testing, TRESS requires a numerical 
contrast (by argument ``Contrast``) as input, in addition
to the output from model fitting. 
The contrast should be based on the name and order of 
coefficients in design matrix. 
...
## Divide genome and obtain candidate DMRs
...
## DMR detection from candidate DMRs 
...
After model fitting, TRESS conducts Wald test for each candidate DMR. As mentioned earlier, the methylation level of each candidate
...
As introduced as the second example dataset in 
