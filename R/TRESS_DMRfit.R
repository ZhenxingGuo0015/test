### diff peak modeling and estimate
TRESS_DMRfit <- function(counts, sf,
                         model, variable,
                         iniby = "lm",
                         shrkPhi = TRUE,
                         addsuedo = FALSE){
  ...
  DMR = list(Ratio = Ratio, loglik = loglik,
             #loglik.MLE = res.MLE$loglik.nb,
             Coef = res.Cov$Coef, Cov = res.Cov$Cov,
             shrkPhi = round(phi, 5), shrkTheta = round(theta,2),
             id.rm = which(!idx))
  print("Done!")
  return(DMR)
}
